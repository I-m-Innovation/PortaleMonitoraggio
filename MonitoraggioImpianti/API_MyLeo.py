...
def get_leo_data(t_start, t_end):
	...
	newDB = {
		"t": pd.to_datetime(curr_df["Stime"]), "PacPV": curr_df["avgPacPV"].astype('float64'),
		"Pbat": curr_df["avgPbat"].astype('float64'),
		"PacGrid": curr_df["avgPacGrid"].astype('float64'), "PacHome": curr_df["avgPacHome"].astype('float64'),
		"IBat": curr_df["Ibat"].astype('float64'),
		"TBat": curr_df["Tbat"].astype('float64'), "VBat": curr_df["Vbat"].astype('float64'),
		"SoC": curr_df["SoC"].astype('float64'), "SoH": curr_df["SoH"].astype('float64'),
		"FreqIn": curr_df["FacIn"].astype('float64'), "FreqOut": curr_df["FacOut"].astype('float64'),
